...
def main():
    ...
    datamanager = torchreid_uncertain.data.ImageDataManager(**imagedata_kwargs(cfg))

    print('Building model-1: {}'.format(cfg.model.name))
    model1 = torchreid_uncertain.models.build_model(
        name=cfg.model.name,
    ...
    optimizer1 = torchreid_uncertain.optim.build_optimizer(
        model1, **optimizer_kwargs(cfg)
    )
    scheduler1 = torchreid_uncertain.optim.build_lr_scheduler(
        optimizer1, **lr_scheduler_kwargs(cfg)
    )

    optimizer2 = torchreid_uncertain.optim.build_optimizer(
        model2, **optimizer_kwargs(cfg)
    )
    scheduler2 = torchreid_uncertain.optim.build_lr_scheduler(
