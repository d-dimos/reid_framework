...
class Engine(object):
    ...
    def _evaluate(
        ...
        def _feature_extraction(data_loader):
            ...
            for batch_idx, data in enumerate(data_loader):
                ...
                features = self.extract_features(imgs, uncertainty)
                batch_time.update(time.time() - end)
        ...
        qf, q_pids, q_camids = _feature_extraction(query_loader, uncertainty=uncertainty)
        print('Done, obtained {}-by-{} matrix'.format(qf.size(0), qf.size(1)))

        print('Extracting features from gallery set ...')
        gf, g_pids, g_camids = _feature_extraction(gallery_loader, uncertainty=uncertainty)
    ...
    def extract_features(self, input, uncertainty=False):
        if uncertainty:
            return self.model(input, return_featuremaps=False, noise_layer=True, eval=True)[2]
        else:
