...
class OSNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1_var = nn.Conv2d(3, channels[0], 7, stride=2, padding=3)
        ...
        if conv1_IN:
            self.bn1 = nn.InstanceNorm2d(channels[0], affine=True)
        else:
    ...
    def featuremaps(self, x, mix=True, eval=False):
        ...
        if mix:
            ...
            lam = beta.sample()
            x = lam * out1_noise.detach() #_o.detach() + (1 - lam) * out1_noise.detach()
        else:
            lam = 0
            x = out1_noise
